...
const createFakeOccasionRepository = (
  ...
  return {
    ...
    save: async (occasion) => {
      store.set(occasion.id, occasion);
      saved.push(occasion);
    },
    get savedEntities() {
      return saved;
...
describe("pledge contribution", () => {
  ...
  const testContributor = getTestContributor({
    walletBalance: createMoney(100, "GBP"),
  });
  ...
  it("deducts from contributor and adds to occasion", async () => {
    ...
    expect(result).toEqual({
      success: true,
      occasion: expect.objectContaining({
        totalPledged: createMoney(25, "GBP"),
      }),
      contributor: expect.objectContaining({
        walletBalance: createMoney(75, "GBP"),
      }),
    });
  ...
  it("rejects pledge when contributor has insufficient balance", async () => {
    const poorContributor = getTestContributor({
      walletBalance: createMoney(10, "GBP"),
    });
    const occasionRepo = createFakeOccasionRepository([testOccasion]);
...
describe("pledgeContribution", () => {
  it("transfers exact amount from contributor to occasion", () => {
    ...
    const contributor = getTestContributor({
      walletBalance: createMoney(100, "GBP"),
    });

    const result = pledgeContribution(
      occasion,
      contributor,
      createMoney(30, "GBP"),
    );
